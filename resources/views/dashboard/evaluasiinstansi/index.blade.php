...
@section('content')
    <div class="flex-1 flex flex-col h-full relative">
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-8 z-10">
            ...
            <div class="bg-red-50 text-maroon px-4 py-1.5 rounded-full text-sm font-bold border border-red-100">
                <i class="fas fa-bell mr-1"></i> {{ $awaitingCount }} Menunggu Pemeriksaan
            </div>
        </header>
        ...
        <main class="flex-1 overflow-y-auto p-8 bg-gray-50 relative">
            ...
            <div class="mb-6 flex flex-wrap gap-2">
                <a href="{{ route('dashboard.evaluasiinstansi.index') }}" class="px-4 py-2 rounded-lg text-xs font-bold transition border {{ !$filterStatus ? 'bg-accent text-white border-accent' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' }}">
                    Semua
                </a>
                <a href="{{ route('dashboard.evaluasiinstansi.index', ['status' => 'siap_diperiksa']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition border {{ $filterStatus == 'siap_diperiksa' ? 'bg-yellow-500 text-white border-yellow-500' : 'bg-white text-yellow-600 border-yellow-200 hover:bg-yellow-50' }}">
                    Siap Diperiksa
                </a>
                <a href="{{ route('dashboard.evaluasiinstansi.index', ['status' => 'revisi']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition border {{ $filterStatus == 'revisi' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-red-600 border-red-200 hover:bg-red-50' }}">
                    Butuh Revisi
                </a>
                <a href="{{ route('dashboard.evaluasiinstansi.index', ['status' => 'selesai']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition border {{ $filterStatus == 'selesai' ? 'bg-green-600 text-white border-green-600' : 'bg-white text-green-600 border-green-200 hover:bg-green-50' }}">
                    Selesai Evaluasi
                </a>
                <a href="{{ route('dashboard.evaluasiinstansi.index', ['status' => 'belum_mengisi']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition border {{ $filterStatus == 'belum_mengisi' ? 'bg-gray-600 text-white border-gray-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' }}">
                    Belum Mengisi
                </a>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-6 py-4 font-semibold text-gray-600 text-sm text-center">Status Berkas</th>
                            <th class="px-6 py-4 font-semibold text-gray-600 text-sm text-center">Aksi Pemeriksaan</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm text-gray-700">
                        @forelse($mappedInstitutions as $row)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                                <td class="px-6 py-4 font-bold text-gray-800">{{ $row['model']->name }}</td>
                                <td class="px-6 py-4 text-center text-gray-600">{{ $row['total_filled'] }} Kriteria</td>
                                <td class="px-6 py-4 text-center font-bold text-accent">{{ number_format($row['total_score'], 2) }}</td>
                                <td class="px-6 py-4 text-center">
                                    <span class="px-3 py-1 font-bold rounded-full text-xs border shadow-sm {{ $row['status_class'] }}">
                                        {{ $row['status_text'] }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($row['status_text'] == 'Belum Mengisi')
                                        <button class="text-gray-300 border border-gray-200 bg-gray-50 px-4 py-2 rounded-lg text-sm font-medium cursor-not-allowed mx-auto" disabled>
                                            Belum Ada Berkas
                                        </button>
                                    @else
                                        <button onclick="openModal('periksaModal-{{ $row['model']->id }}', 'modalBoxPeriksa-{{ $row['model']->id }}')" class="bg-maroon hover:bg-red-900 text-white transition px-4 py-2 rounded-lg text-sm font-medium shadow-md flex items-center gap-2 mx-auto">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-500 italic">Tidak ada instansi binaan dengan kriteria filter tersebut.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @foreach($mappedInstitutions as $row)
        @if($row['status_text'] != 'Belum Mengisi')
            <div id="periksaModal-{{ $row['model']->id }}" class="fixed inset-0 bg-black bg-opacity-60 z-50 hidden flex justify-center items-center backdrop-blur-sm transition-opacity duration-300">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-6xl overflow-hidden transform scale-95 transition-transform duration-300 flex flex-col h-[90vh]" id="modalBoxPeriksa-{{ $row['model']->id }}">
                    
                    <form action="{{ route('dashboard.evaluasiinstansi.store', $row['model']->id) }}" method="POST" class="flex flex-col h-full">
                        @csrf
                        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-white shadow-sm z-10">
                            <div>
                                <h3 class="text-xl font-bold text-gray-800">Evaluasi LKE: <span class="text-accent">{{ $row['model']->name }}</span></h3>
                                <p class="text-xs text-gray-500 mt-1">Nilai Mandiri Sementara: {{ number_format($row['total_score'], 2) }}</p>
                            </div>
                            <button type="button" onclick="closeModal('periksaModal-{{ $row['model']->id }}', 'modalBoxPeriksa-{{ $row['model']->id }}')" class="text-gray-400 hover:text-red-500 transition bg-gray-100 hover:bg-red-50 w-8 h-8 rounded-full flex items-center justify-center">
                                ...
                                @foreach($components as $component)
                                    @foreach($component->subComponents as $sub)
                                        @foreach($sub->criteria as $criteria)
                                            @php 
                                                $eval = $row['evals']->where('lke_criteria_id', $criteria->id)->first(); 
                                                if(!$eval) continue;
                                            @endphp

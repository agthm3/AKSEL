...
@section('content')
    <div class="flex-1 flex flex-col h-full relative">
        ...
        <main class="flex-1 overflow-y-auto p-8 bg-gray-50 relative">
            ...
            @if($isApproved)
                <div class="mb-6 p-5 rounded-2xl bg-gradient-to-r from-teal-600 to-emerald-700 text-white flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl border border-teal-700">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="text-lg font-black tracking-tight">Evaluasi Selesai & Nilai Dikunci</h4>
                            <p class="text-sm text-teal-100 mt-0.5">Seluruh lembar kerja LKE instansi Anda telah selesai divalidasi dan disetujui secara permanen oleh Inspektorat. Hak akses pengisian telah ditutup.</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($isSubmitted)
                <div class="mb-6 p-5 rounded-2xl bg-gradient-to-r from-red-600 to-maroon text-white flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl border border-red-700">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-2xl">
                            <i class="fas fa-lock animate-bounce"></i>
                        </div>
                        <div>
                            <h4 class="text-lg font-black tracking-tight">Lembar Kerja Terkunci Sementara</h4>
                            <p class="text-sm text-red-100 mt-0.5">Dokumen LKE Anda telah dikirim secara formal. Perubahan data dibekukan sampai Pemeriksa atas nama <span class="font-extrabold underline text-white">{{ $namaPemeriksa }}</span> selesai memvalidasi berkas Anda.</p>
                        </div>
                    </div>
                </div>
            @endif

            @if($deadline && !$isSubmitted && !$isApproved)
                <div class="mb-6 p-4 rounded-xl border {{ $isPastDeadline ? 'bg-red-50 border-red-200 text-red-700' : 'bg-yellow-50 border-yellow-200 text-yellow-800' }} flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
                    <div class="flex items-center gap-3">
                        <i class="fas fa-clock text-2xl {{ $isPastDeadline ? 'text-red-500' : 'text-yellow-600' }}"></i>
                        <div>
                            <p class="text-sm font-bold">Batas Waktu Pengisian & Evidence LKE:</p>
                            <p class="text-xs opacity-80">{{ \Carbon\Carbon::parse($deadline)->format('d F Y - H:i') }} WITA</p>
                        </div>
                    </div>
                    <div class="flex gap-4 text-center font-mono" id="countdown-timer">
                        <div><span id="days" class="text-xl font-black block">00</span><span class="text-[10px] uppercase tracking-wider block font-sans">Hari</span></div>
                        <div><span id="hours" class="text-xl font-black block">00</span><span class="text-[10px] uppercase tracking-wider block font-sans">Jam</span></div>
                        <div><span id="minutes" class="text-xl font-black block">00</span><span class="text-[10px] uppercase tracking-wider block font-sans">Menit</span></div>
                        <div><span id="seconds" class="text-xl font-black block">00</span><span class="text-[10px] uppercase tracking-wider block font-sans">Detik</span></div>
                    </div>
                </div>
            @endif
            ...
            <div class="mb-6 flex justify-between items-center">
                ...
                @if($isPastDeadline || $isSubmitted || $isApproved)
                    <button type="button" class="bg-gray-300 text-gray-500 px-5 py-2.5 rounded-lg text-sm font-medium cursor-not-allowed flex items-center gap-2" disabled>
                        <i class="fas fa-lock"></i> Pengisian Terkunci
                    </button>
                @else
                    <button type="button" onclick="openModal('submitModal', 'modalBoxSubmit')" class="bg-maroon hover:bg-red-900 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition shadow-md flex items-center gap-2">
                        ...
                        @forelse($components as $component)
                            <tr class="bg-gray-100 border-b border-gray-200">
                                <td colspan="4" class="px-6 py-3 font-bold text-gray-800">
                                    {{ $component->component_number }}. {{ $component->name }} <span class="text-xs text-gray-500 ml-2 font-normal">(Bobot Maks: {{ $component->weight }})</span>
                                </td>
                            </tr>
                            ...
                            @foreach($component->subComponents as $sub)
                                ...
                                @foreach($sub->criteria as $criteria)
                                    @php $eval = $criteria->evaluations->first(); @endphp
                                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4 align-top text-center">
                                            @if($eval && $eval->predicate)
                                                ...
                                                @if($eval->status == 'disetujui')
                                                    <p class="text-[10px] text-green-600 font-bold mt-1"><i class="fas fa-lock text-[9px]"></i> Disetujui</p>
                                                @elseif($eval->status == 'submitted')
                                                    <p class="text-[10px] text-yellow-600 font-bold mt-1"><i class="fas fa-hourglass-half text-[9px]"></i> Diperiksa</p>
                                                @endif
                                            @else
                                        ...
                                        <td class="px-6 py-4 align-top text-center">
                                            @if($isPastDeadline || $isSubmitted || $isApproved || ($eval && $eval->status == 'disetujui'))
                                                <button type="button" class="text-gray-400 bg-gray-100 border border-gray-200 px-4 py-1.5 rounded text-sm flex items-center gap-2 mx-auto cursor-not-allowed" disabled>
                                                    <i class="fas fa-lock text-xs"></i> Terkunci
                                                </button>
                                            @else
                                                <button type="button" onclick="openModal('evalModal-{{ $criteria->id }}', 'modalBoxEval-{{ $criteria->id }}')" class="{{ $eval && $eval->predicate ? 'text-gray-500 border border-gray-300 bg-white hover:border-accent' : 'bg-accent text-white hover:bg-teal-700' }} transition px-4 py-1.5 rounded text-sm flex items-center gap-2 mx-auto shadow-sm">
                                                    <i class="fas {{ $eval && $eval->predicate ? 'fa-edit' : 'fa-plus' }} text-xs"></i> {{ $eval && $eval->predicate ? 'Ubah' : 'Lengkapi' }}
                                                </button>
                                            @endif
                                        </td>
                                    </tr>
                                    ...
                                    <div class="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
                                        ...
                                        <div>
                                            ...
                                            <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 max-h-40 overflow-y-auto space-y-2">
                                                @foreach($documents as $doc)
                                                    @php $isChecked = $eval && $eval->documents->contains($doc->id); @endphp
                                        ...
                                        <div class="border-t border-gray-100 pt-5">
                                            ...
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                                @foreach(['AA' => '100%', 'A' => '90%', 'BB' => '80%', 'B' => '70%', 'CC' => '60%', 'C' => '50%'] as $pred => $desc)
        ...
        <div id="submitModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex justify-center items-center backdrop-blur-sm transition-opacity duration-300">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform scale-95 transition-transform duration-300" id="modalBoxSubmit">
                <form action="{{ route('dashboard.isipenilaianlke.submit') }}" method="POST">
                    @csrf
                    <div class="p-8 text-center">
                        <div class="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                            <i class="fas fa-paper-plane text-2xl text-maroon"></i>
                        </div>
                        <h3 class="text-xl font-bold text-gray-800 mb-2">Ajukan Evaluasi Ke Inspektorat?</h3>
                        <p class="text-sm text-gray-500 mb-6">Seluruh data kriteria yang Anda simpan akan dikirimkan secara formal untuk diperiksa. Anda tidak dapat mengubah data setelah diajukan ke Inspektorat.</p>
                        <div class="flex justify-center gap-3">
                            <button type="button" onclick="closeModal('submitModal', 'modalBoxSubmit')" class="px-5 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition">Kembali</button>
                            <button type="submit" class="bg-maroon hover:bg-red-900 text-white px-5 py-2.5 rounded-lg text-sm font-bold transition shadow-md">Ya, Ajukan Sekarang</button>
                        </div>
                    </div>
                </form>
            </div>
